...
export const createConsultingFormData = async (req, res) => {
  try {
    const { pageCategory, name, email, phoneNo } = req.body;
    ...
    res.status(201).json({
      message: "Consulting form data added successfully",
      consultingFormEntry,
    });
  } catch (error) {
    res.status(500).json({
      message: "Error creating consulting form data",
      error: error.message,
    });
  }
};
...
export const deleteConsultingFormDataById = async (req, res) => {
  try {
    ...
    if (!form)
      return res.status(404).json({ error: "Consulting form not found" });
    res
      .status(200)
      .json({ message: "Consulting form data deleted successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
